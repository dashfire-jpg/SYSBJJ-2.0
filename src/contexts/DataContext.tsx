...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const unsubReceipts = onSnapshot(collection(db, 'receipts'), (snapshot) => {
      const receiptsData = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as PaymentReceipt));
      if (receiptsData.length > 0) setReceipts(receiptsData);
    });

    // Sincronizar Horários
    const unsubSchedules = onSnapshot(collection(db, 'schedules'), (snapshot) => {
      const schedulesData = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as ClassSchedule));
      if (schedulesData.length > 0) setSchedules(schedulesData.sort((a, b) => a.time.localeCompare(b.time)));
    });

    // Sincronizar QTDs
    const unsubLessonPlans = onSnapshot(collection(db, 'lesson_plans'), (snapshot) => {
      const plansData = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as LessonPlan));
      if (plansData.length > 0) setLessonPlans(plansData);
    });

    // Sincronizar Técnicas
    const unsubTechniques = onSnapshot(collection(db, 'techniques'), (snapshot) => {
      const techniquesData = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as LibraryTechnique));
  ...
  // Persistência automática no Firestore ao adicionar/editar (além do set local)
  ...
  const addStudent = (student: Omit<Student, 'id'>) => {
    ...
    syncToFirestore('students', newStudent);
  ...
  const updateStudent = (id: string, updates: Partial<Student>) => {
    ...
    syncToFirestore('students', updatedStudent);
    logAction('Atualização de Cadastro', `Dados do aluno ID ${id} atualizados`, 'User');
  };

  const deleteStudent = (id: string) => {
    setStudents(prev => prev.filter(s => s.id !== id));
    deleteDoc(doc(db, 'students', id)).catch(err => handleFirestoreError(err, OperationType.DELETE, 'students'));
  ...
  const addPayment = (payment: Omit<Payment, 'id'>) => {
    ...
    syncToFirestore('payments', newPayment);
  ...
  const addReceipt = (receipt: Omit<PaymentReceipt, 'id' | 'status' | 'timestamp'>) => {
    ...
    syncToFirestore('receipts', newReceipt);
  ...
  const approveReceipt = (id: string) => {
    ...
    const updatedReceipt = { ...receipt, status: 'Approved' as const };
    setReceipts(prev => prev.map(r => r.id === id ? updatedReceipt : r));
    syncToFirestore('receipts', updatedReceipt);
  ...
  const rejectReceipt = (id: string) => {
    const receipt = receipts.find(r => r.id === id);
    if (!receipt) return;
    const updatedReceipt = { ...receipt, status: 'Rejected' as const };
    setReceipts(prev => prev.map(r => r.id === id ? updatedReceipt : r));
    syncToFirestore('receipts', updatedReceipt);
  ...
  const addSchedule = (schedule: Omit<ClassSchedule, 'id'>) => {
    ...
    syncToFirestore('schedules', newSchedule);
  ...
  const updateSchedule = (id: string, updates: Partial<ClassSchedule>) => {
    ...
    setSchedules(prev => prev.map(s => s.id === id ? updatedSchedule : s).sort((a, b) => a.time.localeCompare(b.time)));
    syncToFirestore('schedules', updatedSchedule);
  };
  ...
  const addLessonPlan = (plan: Omit<LessonPlan, 'id'>) => {
    ...
    syncToFirestore('lesson_plans', newPlan);
  ...
  const updateLessonPlan = (id: string, updates: Partial<LessonPlan>) => {
    ...
    syncToFirestore('lesson_plans', updatedPlan);
  };

  const deleteLessonPlan = (id: string) => {
    setLessonPlans(prev => prev.filter(p => p.id !== id));
    deleteDoc(doc(db, 'lesson_plans', id)).catch(err => handleFirestoreError(err, OperationType.DELETE, 'lesson_plans'));
  };
  ...
  const addTechnique = (tech: Omit<LibraryTechnique, 'id'>) => {
    ...
    syncToFirestore('techniques', newTech);
  ...
  const updateTechnique = (id: string, updates: Partial<LibraryTechnique>) => {
    ...
    if (!tech) return;
    const updatedTech = { ...tech, ...updates };
    setTechniques(prev => prev.map(t => t.id === id ? updatedTech : t));
    syncToFirestore('techniques', updatedTech);
  };

  const deleteTechnique = (id: string) => {
    setTechniques(prev => prev.filter(t => t.id !== id));
    deleteDoc(doc(db, 'techniques', id)).catch(err => handleFirestoreError(err, OperationType.DELETE, 'techniques'));
  };
  ...
  const addProduct = (product: Omit<Product, 'id'>) => {
    ...
    syncToFirestore('products', newProduct);
  };
  ...
  const updateProduct = (id: string, updates: Partial<Product>) => {
    ...
    syncToFirestore('products', updatedProduct);
  };

  const deleteProduct = (id: string) => {
    setProducts(prev => prev.filter(p => p.id !== id));
    deleteDoc(doc(db, 'products', id)).catch(err => handleFirestoreError(err, OperationType.DELETE, 'products'));
  };
  ...
  const updatePlan = (id: string, updates: Partial<Plan>) => {
    ...
    syncToFirestore('plans', updatedPlan);
  };

  const deletePlan = (id: string) => {
    setPlans(prev => prev.filter(p => p.id !== id));
    deleteDoc(doc(db, 'plans', id)).catch(err => handleFirestoreError(err, OperationType.DELETE, 'plans'));
  };
