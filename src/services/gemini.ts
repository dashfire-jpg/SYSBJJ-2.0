...
// Lazy initialization to avoid startup errors if key is missing
let aiInstance: GoogleGenAI | null = null;

const getAI = (): GoogleGenAI => {
  if (!aiInstance) {
    // In AI Studio, process.env.GEMINI_API_KEY is automatically available.
    // In Vercel/Production, it should be provided via VITE_GEMINI_API_KEY.
    const key = (typeof process !== 'undefined' && process.env?.GEMINI_API_KEY) || import.meta.env.VITE_GEMINI_API_KEY;
    
    if (!key) {
      console.warn("Gemini API Key not found. Please set VITE_GEMINI_API_KEY in environment variables.");
    }
    
    // The error "An API Key must be set when running in a browser" is thrown by the SDK 
    // if key is falsy. We provide a dummy string to prevent the whole app from crashing 
    // on start, allowing the user to see the UI.
    aiInstance = new GoogleGenAI({ apiKey: key || "KEY_NOT_FOUND" });
  }
  return aiInstance;
};
