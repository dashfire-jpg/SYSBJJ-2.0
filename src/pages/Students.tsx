...
const StudentDetailsModal = ({ student, onClose }: { student: Student; onClose: () => void }) => {
  ...
  const handleUpdateAnalysis = async () => {
    await updateStudent(student.id, { pros: editPros, cons: editCons });
    setShowSuccess(true);
    ...
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] w-full max-w-5xl max-h-[95vh] overflow-hidden flex flex-col">
        ...
        <div className="flex gap-2 p-4 bg-slate-100 dark:bg-slate-800 text-xs font-black uppercase">
          {['overview','edit','analysis','admin'].map(tab => (
            <button
              key={tab}
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {activeTab === 'edit' && (
            <form onSubmit={handleUpdateRegistration} className="grid gap-4">
              ...
              <input
                ...
                onChange={e => setEditFormData(prev => ({ ...prev, monthlyValue: safeNumber(e.target.value) }))}
                className="input"
              ...
              <select
                ...
                onChange={e => setEditFormData(prev => ({ ...prev, belt: e.target.value as any }))}
                className="input"
              >
          ...
          {activeTab === 'admin' && (
            <div className="space-y-4">
              <button
                onClick={() => updateStudent(student.id, { status: StudentStatus.INACTIVE })}
