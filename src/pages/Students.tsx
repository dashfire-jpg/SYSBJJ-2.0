    ...
    <div className="space-y-4 max-w-7xl mx-auto pb-12 w-full animate-in fade-in duration-700 overflow-x-hidden">
      ...
      <div className="flex p-1 bg-white dark:bg-slate-800 rounded-xl w-full max-w-[450px] shadow-sm border border-slate-100 dark:border-slate-700">
        ...
        <button 
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${activeView === 'competitors' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-600'}`}
        >
          <Medal size={14}/> {t('students.isCompetitor')}es
        </button>
      </div>

      <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] sm:rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
                ...
                >
                  <td className="px-6 sm:px-8 py-4">
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center font-black transition-all group-hover:rotate-12 shrink-0 overflow-hidden ${student.isKid ? 'bg-yellow-50 text-yellow-600' : 'bg-blue-50 text-blue-600'}`}>
                        {student.photoUrl ? (
                          <img src={student.photoUrl} alt={student.name} className="w-full h-full object-cover" />
                        ) : (
                          student.name[0]
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-black text-slate-900 dark:text-white text-sm sm:text-base tracking-tight uppercase leading-none truncate">{student.name}</p>
                          {student.isCompetitor && <Medal size={12} className="text-blue-600 shrink-0" />}
                        </div>
                        {student.nickname && <p className="text-[8px] text-slate-400 uppercase font-black mt-0.5 italic truncate">"{student.nickname}"</p>}
                      </div>
                    </div>
                  </td>
                  <td className="px-6 sm:px-8 py-4">
                    <span className={`px-2.5 sm:px-3 py-1 rounded-lg text-[8px] sm:text-[9px] font-black uppercase tracking-widest shadow-sm border whitespace-nowrap ${BELT_COLORS[student.belt] || 'bg-slate-100'}`}>
                      {t(`belts.${student.belt}`)}
                    </span>
                  </td>
                  <td className="px-6 sm:px-8 py-4">
